...
const _sfc_main = {
  ...
  setup(__props) {
    const props = __props;
    const hideList = common_vendor.ref(["load-more"]);
    ...
    const goDetailPage = (path) => {
      const url = `/pages/extUI/${path}/${path}`;
      props.hasLeftWin ? common_vendor.index.reLaunch({ url }) : common_vendor.index.navigateTo({ url });
    };
    return (_ctx, _cache) => {
      return common_vendor.e({
        a: !__props.hasLeftWin
      }, !__props.hasLeftWin ? {} : {}, {
        b: !__props.hasLeftWin
      }, !__props.hasLeftWin ? {
        c: common_vendor.p({
        ...
        d: common_vendor.f(lists.value, (item, k0, i0) => {
          return {
            a: common_vendor.t(item.name),
            b: __props.leftWinActive === item.url && __props.hasLeftWin ? 1 : "",
            c: common_vendor.o(($event) => goDetailPage(item.url), item.url),
            d: hideList.value.indexOf(item.url) !== -1 && __props.hasLeftWin ? 1 : "",
            e: item.url
          };
        })
      });
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__file", "/Users/birne/Desktop/mp-weixin/src/pages/tabBar/extUI/extUI.vue"]]);
